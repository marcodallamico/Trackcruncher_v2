...
output_folder = "tables_Nguyen_lowmass"
os.makedirs(output_folder, exist_ok=True)

# Step 2: Define the target folder containing the PARSEC tracks
target_folder = "/home/marco/Desktop/tracks_nguyen/"
# Nskip=0 # for parsec
Nskip=4 # for parsec



# Dictionary to store removed tracks
removed_tracks = {}
        ...
        if new_name in {'0017', '001', '0006'}:                                                                                                                     # CHANGE THIS IS TEMPORARY!!
            ...
            for file_name, masstrack in files_and_masses:
                ...
                if (masstrack < 2. ):
                    ...
                    df, phase_error = phasefinder.determine_phases(df)

                    if phase_error:
                        # Store the masstrack in the dictionary
                        if new_name not in removed_tracks:
                            removed_tracks[new_name] = []
                        removed_tracks[new_name].append(masstrack)
...
# Print removed tracks summary
if removed_tracks:
    print("\nTRACKS WITH PHASE PROBLEMS REMOVED FROM THE TABLES:")
    for z, masses in removed_tracks.items():
        masses_str = ", ".join(map(str, masses))
        print(f"Z={z}, M={masses_str}")
